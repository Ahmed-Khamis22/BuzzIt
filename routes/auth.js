...
router.post('/google', socialAuthLimiter, async (req, res) => {
  try {
    ...
    res.status(err.status || 401).json({
      error: err.status ? err.message : 'تعذر تسجيل الدخول بجوجل، حاول مرة أخرى',
    });
  }
});
...
router.post('/guest', socialAuthLimiter, async (req, res) => {
  try {
    ...
    const requestedName = String(req.body?.username || '').trim();
    if (!requestedName) return res.status(400).json({ error: 'اسم اللاعب مطلوب' });
    if (requestedName.length > 15) return res.status(400).json({ error: 'اسم اللاعب يجب ألا يتجاوز 15 حرفاً' });
    if (await User.exists({ username: requestedName })) {
      return res.status(409).json({ error: 'اسم اللاعب مستخدم بالفعل، اختر اسمًا آخر' });
    }
...
router.post('/google/link', socialAuthLimiter, auth, async (req, res) => {
  try {
    ...
    res.status(err.status || 401).json({
      error: err.status ? err.message : 'تعذر ربط حساب جوجل، حاول مرة أخرى',
    });
  }
});
